...
def checkFor1wOverlap(all_spaces, character_value, opponent_value):
	
	all_winning_combos = checkWinningCombos(all_spaces)
	winning_combos_1char = list()
	potential_play_spaces = list()
	play_space_str = list()
	play_space = list()
	space_count = 0

	for combo in all_winning_combos:
		if combo.count(character_value) == 1 and combo.count(opponent_value) == 0:
			winning_combos_1char.append(combo)

	for combo in winning_combos_1char:
		
		for space in combo:
			if space != character_value:

				for combo in winning_combos_1char:
					if space in combo:
						space_count = space_count + 1
				if space_count > 1:
					potential_play_spaces.append(space)
				space_count = 0

	if potential_play_spaces:
		play_space_str = potential_play_spaces[0].split(',')
...
def turn_AI(all_spaces, character_value):
	if character_value == 'X':
		...
		opponent_value = 'X'
	play_win = checkFor2(all_spaces, character_value)
	if play_win: 
		all_spaces[play_win[0]][play_win[1]] = character_value
	else: 
		block_win = checkFor2(all_spaces, opponent_value)
		if block_win: 
			all_spaces[block_win[0]][block_win[1]] = character_value
		else: 
			play_1of3 = checkFor1wOverlap(all_spaces, character_value, opponent_value)
			if play_1of3:
				all_spaces[play_1of3[0]][play_1of3[1]] = character_value
			else:	
				block_1of3 = checkFor1wOverlap(all_spaces, opponent_value, character_value)
				if block_1of3:
					all_spaces[block_1of3[0]][block_1of3[1]] = character_value
				else:
